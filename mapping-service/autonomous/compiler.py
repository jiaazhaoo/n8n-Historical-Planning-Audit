...
def compiler_prompt(packet_path: Path, packet: dict[str, Any] | None = None) -> str:
...
class CodexOAuthCompiler:
    ...
    def compile(self, *, report: PreparationReport, artifacts: ArtifactStore) -> tuple[MappingSpec, tuple[Path, ...]]:
        ...
        raw_output = artifacts.resolve("compiler/mapping-spec.raw.json")
        if raw_output.exists():
            spec = load_compiled_spec(raw_output)
            canonical = artifacts.write_immutable_json("spec/mapping-spec.json", spec.model_dump(mode="json"))
            return spec, (packet_path, schema_path, raw_output, canonical)
        attempt_output = artifacts.resolve("compiler/mapping-spec.attempt.json")
        ...
        result = self.command_runner(
            ...
            env=self._oauth_environment(),
            input=compiler_prompt(packet_path, packet),
            text=True,
            stdout=subprocess.PIPE,
        ...
        log_path = artifacts.write_mutable("compiler/codex-compiler.log", log_text.encode("utf-8"))
        if result.returncode != 0:
            if attempt_output.exists():
                artifacts.write_mutable("compiler/mapping-spec.failed.json", attempt_output.read_bytes())
                attempt_output.unlink()
            raise CompilerError(f"Codex compiler failed with exit code {result.returncode}; inspect {log_path}")
        ...
        try:
            ...
            failed_path = artifacts.write_mutable(
                "compiler/mapping-spec.failed.json",
                attempt_output.read_bytes(),
            )
            attempt_output.unlink()
            raise CompilerError(f"Codex output failed MappingSpec validation; inspect {failed_path}")
        raw_output = artifacts.write_immutable(
            "compiler/mapping-spec.raw.json",
            attempt_output.read_bytes(),
        )
        attempt_output.unlink()
        canonical = artifacts.write_immutable_json("spec/mapping-spec.json", spec.model_dump(mode="json"))
        return spec, (packet_path, schema_path, raw_output, log_path, canonical)
